...
- 작업 중 메시지 끼워넣기. 봇이 일하는 중에 보낸 메시지는 Claude Code 훅(PostToolUse·Stop)으로 진행 중인 작업에 실시간 반영되고, 끼워 넣지 못한 경우엔 작업이 끝난 직후 자동으로 이어서 처리됩니다. 어느 쪽이든 메시지는 유실되지 않습니다.
- 스레드별 병렬 작업. `/session new`로 나눈 스레드끼리는 서로 기다리지 않고 동시에 돕니다. 스레드마다 별도의 `claude` 프로세스가 자기 대화 맥락을 갖고 움직이므로, 한쪽에서 오래 걸리는 작업을 돌리면서 다른 쪽에 다른 일을 시킬 수 있습니다. 한 스레드 안에서는 여전히 순서대로 하나씩 처리합니다.
- 채팅창 하나에서 여러 스레드를 구분합니다. 답변은 원래 요청에 대한 답장으로 오고, 헷갈릴 상황에서는 첫 줄에 스레드 이름이 붙습니다. 특정 스레드에 말하려면 그 스레드의 메시지에 답장하거나 `@이름 내용`으로 보냅니다. `/session`은 지금 도는 스레드를 표시합니다.
...
| `/remind [30s\|10m\|2h\|1d] 내용` | 리마인더 예약 (`list`/`cancel <id>`) |
