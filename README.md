...
- 스레드별 병렬 작업. 잠금이 채팅이 아니라 채팅과 스레드 조합에 걸려 있어, `/session new`로 나눈 스레드끼리는 서로 기다리지 않고 동시에 돕니다. 한 스레드에서 오래 걸리는 작업을 돌리면서 다른 스레드에 다른 일을 시킬 수 있습니다. 한 스레드 안에서는 여전히 순서대로 하나씩 처리합니다.
